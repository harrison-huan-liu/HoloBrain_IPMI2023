...
def parameter_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=90,
        help="Number of predictive repetitions. Default is 90.",
    )
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help="用 attention 对所有边加权后再随机游走，并且用该 attention 对所有顶点加权后在送入 SPDSRU",
    )
...
class GAMTrainer(torch.nn.Module):
    ...
    def process_graph(self, graph, features, target):
        """
        ...
        for t in range(self.args.time):
            logits, node, attention_score, gam_output = self.model(
                graph, features, node
            )  # call GAM forward
    ...
    def seq_forward(self, seq):
        seq_inputs = []
        loss = 0
        for g, features, target in zip(*seq):
            features = features.reshape(
                -1, self.args.wavelets_num, self.args.wavelets_num
            )
            target = target.reshape(-1)
            graph_loss, graph_output, target = self.process_graph(g, features, target)
            seq_inputs.append(
        ...
        return pred_loss, logits, target
    ...
    def process_seq(self, seq):
        ...
        """
        seq_loss, logits, target = self.seq_forward(seq)
        seq_loss.backward()
        self.optimizer.step()
        loss_value = seq_loss.item()
        self.optimizer.zero_grad()

        return loss_value, logits, target
    ...
    def fit(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            if self.args.mode == "seq":
                ...
                for seq in bar:
                    seq_loss, logits, target = self.process_seq(seq)
        ...
        for b in tqdm(data):
            if self.args.mode == "seq":
                ...
                for repet in range(self.args.repetitions):
                    loss, predictions, target = self.seq_forward(b)
    ...
    def save_predictions_and_logs(self):
        ...
        if self.args.debug:
            ...
            train_steps_file = (
                self.args.model_folder
                + f"trainsteps_{self.args.current_time}_{self.args.mode}.json"
            )
            json.dump(
                self.train_steps,
            ...
            test_steps_file = (
                self.args.model_folder
                + f"teststeps_{self.args.current_time}_{self.args.mode}.json"
            )
            json.dump(
                self.test_steps,
